...
@pytest.mark.parametrize('depth_level, expected_flatten', 
                        [(3, [{'currency': 'EUR', 'value': '49.95'}, {'currency': 'DKK', 'value': '445.60'}]),
                         (0, [{"prices": {"price": [{"currency": "EUR","value": "49.95"},{"currency": "DKK","value": "445.60"}]}}]),
                         (6, []),
                         (4, ['EUR', '49.95', 'DKK', '445.60']),
                        ])
def test_flatten_list(depth_level, expected_flatten):
    input_data = {"prices": {
                        "price": [
                            {
                                "currency": "EUR",
...
def test_flatten_dict(depth_level, expected_flatten):
    input_data = {
                    "prices": [
                        {
                            "currency": "EUR",
